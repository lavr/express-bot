...
- **Мульти-бот** — несколько ботов с автоматическим выбором по чату
- **Секреты** — поддержка переменных окружения и HashiCorp Vault
- **Деплой** — Docker, Helm chart, systemd, бинарник
...
# 2. Добавляем бота (secret обменяется на token автоматически)
...
# Linux (arm64)
curl -sL https://github.com/lavr/express-botx/releases/latest/download/express-botx-linux-arm64.tar.gz | tar xz
sudo mv express-botx /usr/local/bin/
...
Минимальный конфиг (`~/.config/express-botx/config.yaml`):
...
1. **YAML-файл** (`--config`, `EXPRESS_BOTX_CONFIG`, `./express-botx.yaml`, `~/.config/express-botx/config.yaml`)
