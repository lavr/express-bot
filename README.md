...
```mermaid
graph LR
    CLI["CLI<br/><code>express-botx send</code>"] --> BotX["BotX API"]
    API["HTTP-сервер<br/><code>express-botx serve</code>"] --> BotX
    API -->|--enqueue| Queue["RabbitMQ / Kafka"]
    Queue --> Worker["Worker<br/><code>express-botx worker</code>"]
    Worker --> BotX
    BotX --> Express["eXpress"]
    AM["Alertmanager"] -->|webhook| API
    GR["Grafana"] -->|webhook| API
```
