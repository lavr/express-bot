...
- [ ] `Route(event string) []matchedRule` — возвращает все совпавшие правила
- [ ] Конкретные events матчат точно, `"*"` матчит любое событие
- [ ] Правила проверяются в порядке объявления, все совпавшие выполняются
